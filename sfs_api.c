 ...
 * @retval None
 */
char* append_char(char* array, char a, int len)
{
...
int sfs_fopen(char *name) {
    ...
    if(strlen(name) > MAXFILENAME){
        printf("SFS > Filename is too long\n");
        return -1;
    }

    if(strlen(name) == 0){
        printf("SFS > Filename is empty\n");
        return -1;
    }
...
int sfs_fread(int fileID, char *buf, int length) {
    ...
    if(fileID < 0 || length < 0){
        printf("Invalid input for fread\n");
        return -1;
    }
    ...
    if (num_block_read == 1) {
        for (j = start_ptr; j < end_ptr; j++) {
            temp_buf = append_char(temp_buf, buffer[0][j], len_temp_buf);
            len_temp_buf++;
        }
        ...
        for (j = 0; j < end_ptr; j++) {
            temp_buf = append_char(temp_buf, buffer[1][j], len_temp_buf);
            len_temp_buf++;
        }
        ...
        for (j = 1; j < num_block_read - 1; j++) {
            ...
            for (k = 0; k < BLOCK_SZ; k++) {
                temp_buf = append_char(temp_buf, buffer[j][k], len_temp_buf);
                len_temp_buf++;
            }
        }

        for (j = 0; j < end_ptr; j++) {
            temp_buf = append_char(temp_buf, buffer[num_block_read - 1][j], len_temp_buf);
            len_temp_buf++;
        }
    }
...
int sfs_fwrite(int fileID, const char *buf, int length){
    ...
    if(fileID < 0 || length < 0){
        printf("Invalid input for fwrite\n");
        return -1;
    }
    ...
    /*****************************************************************/
        ...
        sfs_frseek(fileID, (f->rwptr - (f->rwptr % BLOCK_SZ)));
        ...
        if (length_left > BLOCK_SZ - (f->rwptr % BLOCK_SZ)) {
            for (i = 0; i < BLOCK_SZ - (f->rwptr % BLOCK_SZ); i++) {
                read_buf = append_char(read_buf, buf[i], len_read_buf);
                len_read_buf++;
                count++;
            }
        } else {
    ...
    for(k = 0; k < num_block_left; k++) {
        ...
        if(length_left > BLOCK_SZ) {
            ...
            for(w = length - length_left; w < length - length_left + BLOCK_SZ; w++){
                write_buf = append_char(write_buf, buf[w], len_write_buf);
                len_write_buf++;
                count++;
            }
...
int sfs_frseek(int fileID, int loc){
    ...
    // error checking 
    if(loc < 0 || loc > MAX_RWPTR || fileID < 0){
